...
for encoder_id in ["bert-base-multilingual-cased", "xlm-roberta-base", "readerbench/RoBERT-base"]:
    ...
    for encoder_freeze in [True, False]:
        ...
        encoder_name = encoder_id + f"-{'encoder_freeze' if encoder_freeze else 'encoder_trained'}" + ("-liquid" if liquid else "")
        ...
        # Initialize model
        model = Classificator(encoder_id, encoder_freeze, liquid)
