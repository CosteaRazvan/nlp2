...
def train(config: ModelConfig, model, train_loader, val_loader, version, class_weights, encoder_freeze):
    ...
    if not config.oversample:
        class_weights = torch.tensor([0.10915805, 0.07564106, 0.03261642, 0.03791726, 0.74466722])
    ...
    for epoch in range(start_epoch+1, config.num_epochs+1):
        ...
        if epoch > 4 and not encoder_freeze:
            torch.save({
